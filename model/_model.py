...
def p_SBL(n, t_path, gamma_path, theta_path):
    ...
    O_nm1 = np.sum(gamma_path * t_path * (1 - np.exp(-theta_path*(n-1))) / (1 - np.exp(-theta_path)))
    O_n = np.sum(gamma_path * t_path * (1 - np.exp(-theta_path*n)) / (1 - np.exp(-theta_path)))
    L_nm1 = np.sum(gamma_path * t_path * np.exp(-theta_path) * (1 - np.exp(-theta_path*(n-1))) / (1 - np.exp(-theta_path)))
    L_n = np.sum(gamma_path * t_path * np.exp(-theta_path) * (1 - np.exp(-theta_path*n)) / (1 - np.exp(-theta_path)))
